...
def apply_erosion(image_path: Path):
    logger.info("Applying erosion in %s", image_path)
    ...
    if res_image_path.exists():
        logger.info("Erosion is already applied, ignoring...")
        return
    ...
    cv2.imwrite(str(res_image_path), eroded_image)
